...
%% set inputs
numClusters = 4;
split=24; % i am using split to denote different processing applied to data 
...
% nperms = 100000;
% permavg = PERM_TEST(PLTransitionProbabilityMatsNoPersist,LSDTransitionProbabilityMatsNoPersist,nperms);
% 
...
h = colorbar; ylabel(h,'PL - LSD'); caxis([-caxis_bound caxis_bound]); h.Ticks = [-caxis_bound 0 caxis_bound]; h.TickLabels = [round(-caxis_bound,2,'significant') 0 round(caxis_bound,2,'significant')];
COLOR_TICK_LABELS(true,true,numClusters);
title('PL - LSD');
set(gca,'FontSize',18);
set(gca,'TickLength',[0 0]);
