%% SI Figure 15 ii: make an energy comparison between LSD and PL data that was clustered separately (split 29/30)
...
%% make a partition for each condition separately

idx = size(concTS,1)/2;

LSD_TS = concTS(1:idx,:);
PL_TS = concTS(idx+1:end,:);

LSDsubjInd = LSDsubjInd(1:idx);
PLsubjInd = PLsubjInd(idx+1:end);

LSDcentroids=NaN(nsubjs,nparc,numClusters);
PLcentroids=NaN(nsubjs,nparc,numClusters);


LSDpartition = kmeans(LSD_TS,numClusters,'Distance', distanceMethod,'Replicates',nreps,'MaxIter',maxI);
PLpartition = kmeans(PL_TS,numClusters,'Distance', distanceMethod,'Replicates',nreps,'MaxIter',maxI);

for i=1:nsubjs
    
    disp(['Generating LSD centroids for subj ',num2str(i)]);
    LSDcentroids(i,:,:) = GET_CENTROIDS(LSD_TS(LSDsubjInd==i,:),LSDpartition(LSDsubjInd==i),numClusters);
    
    disp(['Generating PL centroids subj ',num2str(i)]);
    PLcentroids(i,:,:) = GET_CENTROIDS(PL_TS(PLsubjInd==i,:),PLpartition(PLsubjInd==i),numClusters);
